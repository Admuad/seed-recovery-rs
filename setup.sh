...
LOG_FILE="/tmp/seed-recovery-rs-setup.log"
if ! touch "$LOG_FILE" 2>/dev/null; then
    LOG_FILE="$HOME/seed-recovery-rs-setup.log"
...
if [ "$OS" == "Linux" ]; then
    ...
    if [ ! -d ".git" ] && [ ! -f "Cargo.toml" ]; then
        ...
        REPO_URL="https://github.com/Admuad/seed-recovery-rs.git"

        if [ -d "seed-recovery-rs" ]; then
            echo -e "${YELLOW}Directory 'seed-recovery-rs' exists. Removing and re-cloning...${NC}"
            rm -rf seed-recovery-rs
        fi

        run_animated git clone "$REPO_URL"
        cd seed-recovery-rs || exit 1
    fi
...
echo -e "${YELLOW}Step 3/$TOTAL_STEPS: Project Dependencies${NC}"
...
echo -e "${YELLOW}Step 4/$TOTAL_STEPS: Building Release Binary${NC}"
run_animated cargo build --release
CURRENT_STEP=4
show_progress $CURRENT_STEP $TOTAL_STEPS

echo -e "${YELLOW}Step 5/$TOTAL_STEPS: Installation${NC}"
mkdir -p "$HOME/.local/bin"
cp target/release/seed-recovery-rs "$HOME/.local/bin/seed-recovery-rs"
chmod +x "$HOME/.local/bin/seed-recovery-rs"
...
if [ "$PWD" != "$ORIGINAL_DIR" ]; then
    RUN_CMD="cd seed-recovery-rs && ./target/release/seed-recovery-rs"
    GLOBAL_CMD="seed-recovery-rs"
else
    RUN_CMD="./target/release/seed-recovery-rs"
    GLOBAL_CMD="seed-recovery-rs"
fi
...
echo ""
echo -e "Or from anywhere (after reloading your shell):"
echo -e "${YELLOW}  $GLOBAL_CMD${NC}"
echo ""
echo -e "${CYAN}Note: You may need to run 'source ~/.bashrc' or restart your terminal${NC}"
echo -e "${CYAN}      for the PATH changes to take effect.${NC}"
echo ""
